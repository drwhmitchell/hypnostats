...
function CreateHypnoChart(chartContainerID, titleText, width, startTime, endTime, sleepArch) {
 ...
 // const skipped = (ctx, value) => {console.log("Skipped (" + ctx.p0.parsed.y + "," + ctx.p1.parsed.y + ")"); return(ctx.p0.skip || ctx.p1.skip ? value : undefined);};
  ...
  newHTMLbuf += "<canvas id='hypnogram-canvas' width='" + width + "' height='" + width*.5 + "'></canvas>";
...
function CreateStatsChart(chartContainerID, titleText, width, startTime, endTime, sleepArch) {
 ...
 // newHTMLbuf += "<canvas id='" + newChartElID + "' style='width:400px; height:125px;'></canvas>";
  ...
  newHTMLbuf += "<canvas id='stats-canvas' width='" + width + "' height='" + width*0.35 + "'></canvas>";
